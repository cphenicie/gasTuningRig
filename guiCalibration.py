...
from pyqtShortcuts import MatplotlibWidget
...
updateTime = 200  # Time between consecutive calls of the readData function in ms. (The actual time between data points might be a little longer than this, graphing is a slow operation...)
### END USER INPUT #####
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.matplotlibWidget.axis.set_ylabel('Random number (a.u.)')
        ...
        self.graphTimer.start(updateTime)  # Update every second
        self.show()
        self.dataArr = list()
        self.timeArr = list()
    ...
    def writeDataFile(self, dataFileName):
        dataShortcuts.writeCSV(dataFileName, [self.timeArr, self.dataArr], ["Time(sec)", "Number(a.u.)"])
